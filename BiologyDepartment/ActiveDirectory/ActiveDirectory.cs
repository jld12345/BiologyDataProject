...
namespace BiologyDepartment.Active_Directory
{
        public class DaoActiveDirectory
        {
            ...
            public void ChangeUserPassword(string sUserName, string sNewPassword, out string sMessage)
            {
                try
                {
                    sMessage = "";
                    ...
                    SearchResult sr = search.FindOne();
                        if (sr != null)
                        {
                            ...
                            e2.Username = GlobalVariables.ADUserName;
                            e2.Password = GlobalVariables.ADPass;

                            if (e2.NativeGuid != null)
                            {
                                ResetPassword(e2, GlobalVariables.ADPass, sNewPassword);
                            }
                            GlobalVariables.ADPass = sNewPassword;
                            sMessage = "Password has been changed.";
                        }
                    }
                }
            ...
            public PrincipalContext GetPrincipalContext(string sUserName, string sPassword)
            {
                try
                {
                    ...
                    PrincipalContext theContext = new PrincipalContext(ContextType.Domain, GlobalVariables.ActiveDirectoryConnection,
                        "PostgresDB\\" + sUserName, sPassword);
                    sw.Stop();
